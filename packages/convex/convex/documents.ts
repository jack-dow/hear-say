...
export const initUpload = zAuthMutation({
	...
	handler: async (ctx, { docId, filename }) => {
		const existing = await ctx.db
			.query('documents')
			.withIndex('by_user_docId', (q) => q.eq('userId', ctx.userId).eq('docId', docId))
			.first()
...
export const savePosition = zAuthMutation({
	...
	handler: async (ctx, { docId, sentenceId }) => {
		const existing = await ctx.db
			...
			.withIndex('by_user_doc', (q) => q.eq('userId', ctx.userId).eq('docId', docId))
			.first()
...
export const deleteDocument = zAuthMutation({
	...
	handler: async (ctx, { documentId }) => {
		...
		const pos = await ctx.db
			.query('readingPositions')
			.withIndex('by_user_doc', (q) => q.eq('userId', ctx.userId).eq('docId', doc.docId))
			.first()
