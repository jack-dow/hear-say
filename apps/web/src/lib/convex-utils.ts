...
function handleConvexError(error: unknown): ConvexCallError {
	...
	const message = isConvex ? String(error.data?.message ?? error.data) : 'An unexpected error occurred'
...
export function useSafeMutation<Mutation extends FunctionReference<'mutation'>>(
	...
	const safe = async (args: Mutation['_args']): Promise<FunctionReturnType<Mutation> | ConvexCallError> => {
		tracker.start()
		try {
...
export function useSafeAction<Action extends FunctionReference<'action'>>(action: Action): SafeAction<Action> {
	const tracker = useLoadingTracker()
	const act = useAction(action)

	const safe = async (args: Action['_args']): Promise<FunctionReturnType<Action> | ConvexCallError> => {
		tracker.start()
		try {
