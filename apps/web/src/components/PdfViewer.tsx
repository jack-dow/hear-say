...
export function PdfViewer({ url, activePage, activeBbox, sentenceOverlays, onAnnotate }: Props) {
	const [ReactPdf, setReactPdf] = React.useState<typeof import('react-pdf') | null>(null)
	...
	return (
		<div ref={containerRef} className="h-full w-full">
			...
			{Document && Page && (
				<motion.div initial={{ opacity: 0 }} animate={{ opacity: pdfLoading ? 0 : 1 }} transition={{ duration: 0.25 }}>
					<Document
						file={url}
						onLoadSuccess={(data) => {
									...
									{overlays?.map((overlay) => (
										<div
											...
											style={{
												...
												backgroundColor: overlay.highlightColor ? `${overlay.highlightColor}66` : undefined,
										...
										>
											...
											{!overlay.highlightColor && !overlay.isBookmarked && (
												<div className="hover:bg-primary/8 hover:outline-primary/20 absolute inset-0 rounded opacity-0 transition-opacity group-hover:opacity-100 hover:outline" />
											)}
											{/* Bookmark star indicator */}
											{overlay.isBookmarked && (
												<StarIcon
													weight="fill"
													className="absolute -top-2 -right-1.5 size-3 text-amber-400 drop-shadow-sm"
												/>
											)}
										</div>
									))}
