...
export function AudioPlayer({
	...
	const currentPage = pages.find((p) => p.sentences.some((s) => s.id === activeSentenceId))?.page ?? 1
	...
	return (
		<div className="bg-background sticky bottom-0 border-t">
			<div className="mx-auto flex max-w-3xl items-center gap-3 px-6 py-3">
				...
				{/* Sentence nav + play */}
				<Button aria-label="Previous sentence" size="icon" variant="ghost" onClick={handlePrev}>
					<SkipBackIcon weight="thin" />
				</Button>
				<Button
				...
				>
					{isPlaying ? (
						<PauseIcon weight="thin" />
					) : isBuffering ? (
						<Spinner className="size-4" />
					) : (
						<PlayIcon weight="thin" />
					)}
				</Button>
				<Button aria-label="Next sentence" size="icon" variant="ghost" onClick={handleNext}>
					<SkipForwardIcon weight="thin" />
				</Button>
				...
				<select
					...
					className={cn(
						'border-input bg-background border px-2 py-1 text-sm',
