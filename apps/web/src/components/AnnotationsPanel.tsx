...
import { Bookmark, DownloadSimple, Highlighter, NavigationArrow, Trash } from "@phosphor-icons/react";
          ...
          {annotations.length === 0 ? (
            ...
            <div className="flex flex-col gap-6">
              {bookmarks.length > 0 && (
                <section>
                  <div className="mb-2 flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
                    <Bookmark weight="thin" className="size-3" />
                    Bookmarks
                  </div>
                  <ul className="flex flex-col gap-2">
                    {bookmarks.map((b) => (
                      <AnnotationItem
                        ...
                        onRemove={() =>
                          removeAnnotation({ annotationId: b._id as Id<"annotations"> })
                        }
                        onNoteChange={(note) =>
                          updateNote({ annotationId: b._id as Id<"annotations">, note })
                        }
                      />
                    ))}
                  </ul>
              ...
              {highlights.length > 0 && (
                <section>
                  <div className="mb-2 flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
                    <Highlighter weight="thin" className="size-3" />
                    Highlights
                  </div>
                  <ul className="flex flex-col gap-2">
                    {highlights.map((h) => (
                      <AnnotationItem
                        ...
                        onRemove={() =>
                          removeAnnotation({ annotationId: h._id as Id<"annotations"> })
                        }
                      />
                    ))}
                  </ul>
...
function AnnotationItem({
  ...
  return (
    <li className="group flex flex-col gap-1 rounded-md border bg-muted/40 p-2.5">
      <div className="flex items-start gap-1">
        ...
        <p className="flex-1 text-xs leading-relaxed text-foreground line-clamp-3">
          {text || annotation.sentenceId}
        </p>
        <div className="flex shrink-0 opacity-0 transition-opacity group-hover:opacity-100">
          <Button
            ...
            className="size-6"
          >
            <NavigationArrow weight="thin" className="size-3" />
          </Button>
          <Button
            size="icon-sm"
            variant="ghost"
          ...
          >
            <Trash weight="thin" className="size-3" />
          </Button>
        </div>
      </div>
      {annotation.type === "bookmark" && onNoteChange && (
        <textarea
          className="min-h-12 w-full resize-none rounded border bg-transparent px-1.5 py-1 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring"
