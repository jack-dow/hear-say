...
import { FileTextIcon, SignOutIcon, SparkleIcon, TagIcon, TrashIcon } from '@phosphor-icons/react'
...
function Home() {
	...
	const handleFile = React.useCallback(
		async (file: File) => {
			...
			navigate({ to: '/reader/$docId', params: { docId }, search: { view: 'text' } })
		},
		[token, navigate],
	)
		...
		<main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-10 px-6 py-16">
			<div className="flex items-start justify-between">
				<div className="space-y-2">
					...
					<p className="text-muted-foreground">Upload a PDF to read it aloud with sentence-by-sentence highlighting.</p>
				</div>
				<ThemeToggle />
				<Button variant="ghost" size="icon-sm" aria-label="Sign out" onClick={() => signOut()}>
					<SignOutIcon weight="thin" />
				</Button>
			</div>
			...
			{recentDocs && recentDocs.length > 0 && (
				<section className="space-y-3">
					...
					<h2 className="text-muted-foreground text-sm font-medium tracking-widest uppercase">Recent</h2>
						...
						{recentDocs.map((doc) => (
							<li key={doc._id} className="group flex items-center gap-1">
								...
								{doc.status === 'ready' && !doc.llmCleaned && token && (
									...
									>
										<SparkleIcon weight="thin" className="text-muted-foreground size-4" />
									</Button>
								)}
								<Button
									variant="ghost"
