...
import { FileText, Tag, Trash } from "@phosphor-icons/react";
...
function TagPage() {
  ...
  const childTags = React.useMemo(
    () => allTags?.filter((t) => t.parentId === tagId) ?? [],
    [allTags, tagId],
  );
    ...
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-8 px-6 py-16">
          ...
          <div className="flex flex-wrap gap-1.5">
            {childTags.map((child) => (
              <Link
                ...
                className="inline-flex items-center gap-1.5 rounded-md bg-muted px-2.5 py-1 text-xs font-medium text-muted-foreground transition-colors hover:bg-muted/80"
              >
      ...
      <section className="space-y-3">
        ...
        {filteredDocs.length === 0 ? (
          <p className="px-3 text-sm text-muted-foreground">No documents in this folder.</p>
        ) : (
                  ...
                  {doc.status === "processing" && (
                    <span className="text-xs text-muted-foreground">
                      {doc.retryCount && doc.retryCount > 0
                        ...
                        : ({ converting: "Extracting", ocr: "OCR", cleaning: "Cleaning" }[
                            doc.processingStep ?? ""
                          ] ?? "Uploading")}
                    </span>
                  )}
